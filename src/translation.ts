export function parseTranslationData(data: {
  [index: string]: any;
}): TranslationKeys {
  return Object.keys(data).reduce<TranslationKeys>((keys, key) => {
    keys[key] = parseTranslationKey(key, data[key]);
    return keys;
  }, {});
}

export function simplifyTranslationKeys(data: TranslationKeys): {
  [index: string]: any;
} {
  return Object.keys(data).reduce<{ [index: string]: any }>((keys, key) => {
...
export type TranslationKeyContextItem = {
  value: TranslationKeyValue;
  matches: TranslationKeyContextMatches;
};
export type TranslationKeyContextMatches = {
